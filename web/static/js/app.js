...
class EasilyPanel5App {
    ...
    // 数据更新方法
    updateJavaStatus(response) {
        // 处理标准响应格式
        const javaInfos = response.success ? response.data : [];
        const hasValidJava = javaInfos && javaInfos.length > 0 && javaInfos.some(info => info.is_valid);

        this.systemStatus.java = hasValidJava ? 'available' : 'unavailable';

        const javaStatusEl = document.getElementById('javaStatus');
        if (javaStatusEl) {
            if (hasValidJava) {
                const validJava = javaInfos.find(info => info.is_valid);
                javaStatusEl.textContent = `Java ${validJava.version}`;
    ...
    updateServersData(response) {
        // 处理标准响应格式
        const servers = response.success ? response.data : [];
        this.servers.clear();
        let runningCount = 0;
        let stoppedCount = 0;
    ...
    updateDaemonStatus(response) {
        // 处理标准响应格式
        const status = response.success ? response.data : {};
